...
export default function KanjiListCard({ kanjiList }: KanjiListCardProps) {
  ...
  return (
    <>
      <ScrollArea className={`max-w-max rounded-md border ${displayList ? 'h-72' : 'h-auto'}`}>
        <div className="p-4">
          <button onClick={() => setDisplayList(!displayList)} className="mb-4 text-sm font-medium leading-none">{!displayList ? 'View Kanji' : 'Hide Kanji'}</button>
          {displayList ? (
            <ul>{kanjiList.map((kanji, index) => (
                  ...
                  <HoverCardContent className="w-full">
                    <div className="flex justify-between space-x-4">
                      <div className="space-y-1">
                        <h4 className="text-sm font-semibold">{kanji.character}</h4>
                        {kanji.readings.map((reading, idx) => (
                          <li className="list-none text-sm" key={`${reading}-${idx}`}>
                            {reading}
                          </li>))}
